...
class ConnectionSpec internal constructor(
  ...
  private var lazyCipherSuites: List<CipherSuite>? = null
  private var lazyTlsVersions: List<TlsVersion>? = null
  ...
  @get:JvmName("cipherSuites") val cipherSuites: List<CipherSuite>?
    get() {
      var result = lazyCipherSuites
      if (result == null) {
        result = cipherSuitesAsString?.map { CipherSuite.forJavaName(it) }?.toList()
        lazyCipherSuites = result
      }
      return result
    }
  ...
  @get:JvmName("tlsVersions") val tlsVersions: List<TlsVersion>?
    get() {
      var result = lazyTlsVersions
      if (result == null) {
        result = tlsVersionsAsString?.map { TlsVersion.forJavaName(it) }?.toList()
        lazyTlsVersions = result
      }
      return result
    }
  ...
  class Builder {
    ...
    constructor(connectionSpec: ConnectionSpec) {
      ...
      this.cipherSuites = connectionSpec.cipherSuites?.map { it.javaName }?.toTypedArray()
      this.tlsVersions = connectionSpec.tlsVersions?.map { it.javaName }?.toTypedArray()
